...
;(function (global, factory) {
  ...
  function updateSlidesProgress(translate) {
    ...
    for (let i = 0; i < slides.length; i += 1) {
      ...
      const slideProgress =
        (offsetCenter + (params.centeredSlides ? swiper.minTranslate() : 0) - slideOffset) / (slide.swiperSlideSize + params.spaceBetween)
  ...
  function Pagination(_ref) {
    ...
    on('click', (_s, e) => {
      ...
      if (
        ...
        if (
          swiper.navigation &&
          ((swiper.navigation.nextEl && targetEl === swiper.navigation.nextEl) || (swiper.navigation.prevEl && targetEl === swiper.navigation.prevEl))
        )
          return
  ...
  function Scrollbar(_ref) {
    ...
    function setDragPosition(e) {
      ...
      let positionRatio
      positionRatio =
        (getPointerPosition(e) - $el.offset()[swiper.isHorizontal() ? 'left' : 'top'] - (dragStartPos !== null ? dragStartPos : dragSize / 2)) /
        (trackSize - dragSize)
    ...
    const updateSlide = (i, slide, slidesLength, getDirectionLabel) => {
      ...
      if (fill === 'row' && slidesPerGroup > 1) {
        ...
        const columnsInGroup =
          groupIndex === 0 ? slidesPerGroup : Math.min(Math.ceil((slidesLength - groupIndex * rows * slidesPerGroup) / rows), slidesPerGroup)
      ...
      if (params.shadow) {
        if (isHorizontal) {
          $cubeShadowEl.transform(
            `translate3d(0px, ${swiperWidth / 2 + params.shadowOffset}px, ${-swiperWidth / 2}px) rotateX(90deg) rotateZ(0deg) scale(${
              params.shadowScale
            })`
          )
        } else {
      ...
      for (let i = 0; i < slides.length; i += 1) {
        ...
        const scaleString =
          originalProgress < 0
            ...
            : `scale(${1 - (1 - data.scale) * originalProgress * multiplier})`
        const opacityString =
          originalProgress < 0 ? 1 + (1 - data.opacity) * originalProgress * multiplier : 1 - (1 - data.opacity) * originalProgress * multiplier
